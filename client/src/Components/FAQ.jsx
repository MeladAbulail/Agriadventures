...
const FAQ = () => {
  ...
  return (
    <section className="bg-[#fcf9f3] text-[#224229] ">
      <div className="max-w-screen-xl px-4 py-8 mx-auto sm:py-16 lg:px-6">
        <h2 className="mb-8 text-4xl font-extrabold tracking-tight text-[#224229] ">Frequently asked questions</h2>
        <div className="grid pt-8 text-left border-t border-gray-200 md:gap-16 md:grid-cols-2">
          <div>
            {currentItems.slice(0, 5).map((item) => (
              <div key={item.id} className="mb-10">
                <h3 className="flex items-center mb-4 text-lg font-bold	 text-[#224229] ">
                ...
                <p className="text-[#224229] font-medium	">
                  {item.answer}
                </p>
              </div>
            ))}
      ...
      <div className="flex justify-center mt-8 mb-8">
        {Array.from({ length: Math.ceil(questions.length / itemsPerPage) }, (_, index) => (
          <button key={index} onClick={() => paginate(index + 1)} className="px-3 py-2 mx-1 bg-[#224229]  text-[#fcf9f3] rounded-lg">
            {index + 1}
          </button>
        ))}
      </div>
